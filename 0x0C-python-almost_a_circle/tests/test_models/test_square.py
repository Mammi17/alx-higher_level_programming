...
class Test_Square_display(TestCase):
    ...
    def test_display_with_args(self):
        with self.assertRaises(TypeError):
            Rectangle(2).display(43)

class Test_Square_str(TestCase):
    """Test cases for Square's _str__ method."""

    def setUp(self):
        Base._Base__nb_objects = 0

    def test_str_print(self):
        expected = "[Square] (43) 1/3 - 2"
        with mock.patch('sys.stdout', new=StringIO()) as output:
            print(Square(2, 1, 3, 43), end='')
...
class Test_Square_update_with_args(TestCase):
    ...
    def test_update_no_args(self):
        s = Square(2, 1, 2, 43)
        s.update()
        self.assertEqual(str(s), "[Square] (43) 1/2 - 2")
    ...
    def test_update_args_width_getter(self):
        s = Square(2, 1, 2, 43)
        s.update(10, 43, 10, 10, 50, 60)
        self.assertEqual(s.width, 43)
...
class Test_Square_update_with_kwargs(TestCase):
    ...
    def test_update_kwargs_args_before(self):
        s = Square(2, 1, 2, 43)
        s.update(43, 43, y=10, x=30)
        self.assertEqual(str(s), "[Square] (43) 1/2 - 43")

    def test_update_kwargs_not_all_mixed(self):
        s = Square(2, 1, 2, 43)
        s.update(y=10, x=10, size=10)
        self.assertEqual(str(s), "[Square] (43) 10/10 - 10")
